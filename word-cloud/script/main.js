...
// second example
const source = await d3.text('./data/palabras2021.txt', d3.autoType);
...
const words = source.split(/[\s.]+/g)
  .map(w => w.replace(/^[“‘"\-—()\[\]{}]+/g, '')) // remove leading punctuation
  .map(w => w.replace(/[;:.!?()\[\]{},"'’”\-—]+$/g, '')) // remove trailing punctuation
  // .map(w => w.replace(/['’]s$/g, '')) // remove possessive
  // .map(w => w.substring(0, 200)) // truncate to 200 characters
  // .map(w => w.toLowerCase())
